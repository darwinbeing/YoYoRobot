...
int flag = 0;
string result;
bool speechingFlag = true; //default true, continue recording
...
void startRecordRecognise()
{
	int ret = MSP_SUCCESS;

	/* login params, please do keep the appid correct */
	const char* login_params = "appid = 57207923, work_dir = .";
...
void asrCallback(const std_msgs::Int32::ConstPtr& msg)
{
    if(WAKEUP == msg->data)
    {
        cout<<"get wakeup flag,now start record ..." << endl;
        startRecordRecognise();
    }
}
...
int main(int argc, char* argv[])
{
    ...
    ros::Subscriber subRecord = ndHandle.subscribe("/voice/xf_asr_topic", 5, asrCallback);
    ros::Publisher pub = ndHandle.advertise<std_msgs::String>("/voice/tuling_nlp", 5);
