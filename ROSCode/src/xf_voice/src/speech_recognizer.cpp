...
#define DEFAULT_SESSION_PARA "sub = iat, domain = iat, language = zh_cn, accent = mandarin, sample_rate = 16000, result_type = plain, result_encoding = utf8"
...
{
	char *s;
    const char* sample = "sample_rate";
    const char* equal = "=";
	if ((s = strstr((char *)session_para, sample)))
    {
		s = strstr(s, equal);
		if (s && *s) {
			s = skip_space(s);
...
int sr_init_ex(struct speech_rec * sr, const char * session_begin_params,
			enum sr_audsrc aud_src, record_dev_id devid, struct speech_rec_notifier * notify)
{
	int errcode;
	size_t param_size;
	...
	if ((aud_src == SR_MIC) && (a == 0))
    {
        cout << "aud_src == SR_MIC or get_input_dev_num == 0 error !!!" << endl;
		return -E_SR_NOACTIVEDEVICE;
	}

    if(sr == NULL)
    {
        cout << "sr pointer is NULL !!!!" << endl;
    }

	if (!sr)
		return -E_SR_INVAL;

    cout << "after judge if(!sr) ..." <<endl;
...
fail:
    cout << "sr_init_ex() function fail ..." << endl;
	if (sr->recorder) {
...
{
    cout << "sr_start_listening() function begin ..." <<endl;
	int ret;
    ...
    cout << "sr_start_listening() function end ..." <<endl;
